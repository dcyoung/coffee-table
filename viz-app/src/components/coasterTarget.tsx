  ...
  switch (importName) {
    case IMPORT_NAME_MONTEREY:
      return [
        ...
        new Quaternion().setFromEuler(
          new Euler(-Math.PI / 2, Math.PI / 2, -Math.PI / 2, "YXZ")
        ),
        new Quaternion().setFromAxisAngle(new Vector3(0, 1, 0), 0),
      ];
    case IMPORT_NAME_SF:
      return [
        ...
        new Quaternion().setFromEuler(
          new Euler(Math.PI / 2, 0, Math.PI / 2, "ZYX")
        ),
        new Quaternion().setFromEuler(new Euler(0, Math.PI / 2, 0, "YXZ")),
        new Quaternion().setFromAxisAngle(new Vector3(0, 1, 0), 0),
      ];
...
const CoasterTarget = ({
  ...
  const CoasterComponent = React.lazy(
    () => import(`./coasters/${importName}/model.tsx`)
  );
  return (
    <Suspense fallback={<CoasterPlaceholder {...props} />}>
      {scrollable ? (
        <ScrollReactiveModel
          orientationSequence={getScrollOrientationSequence(importName)}
        >
